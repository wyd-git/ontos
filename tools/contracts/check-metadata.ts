...
export async function runMetadataContractChecks(
  ...
  if (activeResourceFamilyCount !== 2 || deferredResourceFamilyCount !== 8) {
    throw new Error("G2-01 Resource Family Registry activation boundary drifted.");
  }
...
function validateMetadataCatalog(
  ...
  for (const family of families) {
    ...
    const expectedFrozen = name === "ResourceRevisionReleasePackage";
    if (family.fieldsFrozen !== expectedFrozen) {
      throw new Error(`${name} fieldsFrozen does not match the G2-01 activation boundary.`);
    }
  }
}
