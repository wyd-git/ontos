      ...
      await withClient(adminConfig, async (admin) => {
        ...
        assert.deepEqual(
          ...
          [
            ...
            "0007_materialization_control_facts.sql",
            "0008_materialization_shared_projection.sql",
            "0009_materialization_operations.sql",
            "0010_managed_csv_ingress.sql",
            "0011_object_identity_base_staging.sql",
            "0012_materialization_quality_current.sql",
            "0013_materialization_job_worker.sql",
            "0014_projection_index_capacity_ddl.sql",
            "0015_runtime_plan_compatibility.sql",
            ...
            "0022_query_policy_identity_facts.sql",
            "0023_query_lease_gc_boundary.sql",
...
async function assertLedger(client: pg.Client): Promise<void> {
  const definitions = await loadMigrationDefinitions(databaseMigrationDirectory);
  const result = await client.query<{
...
async function assertDb01Catalog(client: pg.Client): Promise<void> {
  const expectedRelations = [
    ...db01AuthzTables.map((tablename) => `authz.${tablename}`),
    ...db01MetaTables.map((tablename) => `meta.${tablename}`),
  ];
...
async function assertDb01ScopeBoundary(client: pg.Client): Promise<void> {
  const deferredTables = await client.query<{
    ...
    FROM pg_catalog.pg_tables
    WHERE schemaname IN ('action', 'audit')
    ORDER BY schemaname, tablename`);
  assert.deepEqual(deferredTables.rows, [
    { schemaname: "audit", tablename: "claim_mapping_activation_events" },
  ]);
}
...
async function exerciseForwardRepair(client: pg.Client): Promise<void> {
  ...
  const seventhMigration = resolve(directory, "0007_materialization_control_facts.sql");
  const eighthMigration = resolve(directory, "0008_materialization_shared_projection.sql");
  const ninthMigration = resolve(directory, "0009_materialization_operations.sql");
  const tenthMigration = resolve(directory, "0010_managed_csv_ingress.sql");
  const eleventhMigration = resolve(directory, "0011_object_identity_base_staging.sql");
  const twelfthMigration = resolve(directory, "0012_materialization_quality_current.sql");
  const thirteenthMigration = resolve(directory, "0013_materialization_job_worker.sql");
  const fourteenthMigration = resolve(directory, "0014_projection_index_capacity_ddl.sql");
  const fifteenthMigration = resolve(directory, "0015_runtime_plan_compatibility.sql");
  ...
  const twentySecondMigration = resolve(directory, "0022_query_policy_identity_facts.sql");
  const twentyThirdMigration = resolve(directory, "0023_query_lease_gc_boundary.sql");
  const twentyFourthMigration = resolve(directory, "0024_query_policy_authorization_boundary.sql");
  ...
  const failedMigration = resolve(directory, "0027_failed_attempt.sql");
  const defectMigration = resolve(directory, "0027_forward_repair_probe.sql");
  const repairMigration = resolve(directory, "0028_forward_repair.sql");

  try {
    await copyFile(resolve(databaseMigrationDirectory, "0001_foundation.sql"), firstMigration);
    await copyFile(
      resolve(databaseMigrationDirectory, "0002_metadata_control_plane.sql"),
      secondMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0003_resource_revision_guards.sql"),
      thirdMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0004_dependency_validation_guards.sql"),
      fourthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0005_release_lifecycle_guards.sql"),
      fifthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0006_package_lifecycle_guards.sql"),
      sixthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0007_materialization_control_facts.sql"),
      seventhMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0008_materialization_shared_projection.sql"),
      eighthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0009_materialization_operations.sql"),
      ninthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0010_managed_csv_ingress.sql"),
      tenthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0011_object_identity_base_staging.sql"),
      eleventhMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0012_materialization_quality_current.sql"),
      twelfthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0013_materialization_job_worker.sql"),
      thirteenthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0014_projection_index_capacity_ddl.sql"),
      fourteenthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0015_runtime_plan_compatibility.sql"),
      fifteenthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0016_snapshot_group_cutover.sql"),
      sixteenthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0017_generation_index_gc.sql"),
      seventeenthMigration,
    );
    await copyFile(
    ...
    await copyFile(
      resolve(databaseMigrationDirectory, "0022_query_policy_identity_facts.sql"),
      twentySecondMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0023_query_lease_gc_boundary.sql"),
      twentyThirdMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0024_query_policy_authorization_boundary.sql"),
      twentyFourthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0025_runtime_identity_boundary.sql"),
      twentyFifthMigration,
    );
    await copyFile(
      resolve(databaseMigrationDirectory, "0026_policy_resource_compiler_release_gate.sql"),
      twentySixthMigration,
    );
    await writeFile(
      failedMigration,
    ...
    await assertProbeAndLedgerState(client, false, 26);
    ...
    await assertProbeAndLedgerState(client, true, 28);
  } finally {
