...
export interface G20305EvidencePolicy {
  readonly schemaVersion: 1;
  readonly gate: "G2-03-05";
...
export interface G20305EvidenceSnapshot {
  ...
  readonly policyCompilerArtifact: unknown;
}
...
const requiredArtifactAssertions = Object.freeze([
  "exactDependencies",
  "missingCompilationBlocked",
  "apiCompilationWriteDenied",
  "directDatabaseBypassBlocked",
  "forgedCompilationBlocked",
  "wrongBindingCompilationBlocked",
  "artifactsDigestVerified",
  "migration26RollsBack",
  "releasePublished",
  "compilationImmutable",
]);

export function evaluateG20305EvidenceSnapshot(
  snapshot: G20305EvidenceSnapshot,
  policy: G20305EvidencePolicy,
): readonly string[] {
  assertPolicy(policy);
  ...
  for (const path of snapshot.changedFiles) {
    if (policy.scope.forbiddenPrefixes.some((prefix) => path.startsWith(prefix))) {
      violations.push(`G2-03-05 forbids changed path ${path}.`);
      continue;
    }
    if (
      ...
      violations.push(`G2-03-05 does not allow changed path ${path}.`);
    }
  }
  ...
  validatePolicyCompilerArtifact(
    snapshot.policyCompilerArtifact,
...
export function g20305EvidenceManifest(
  report: Readonly<Record<string, unknown>>,
  acceptance: Readonly<Record<string, unknown>>,
  policyCompilerArtifact: unknown,
  ...
  const artifactPass =
    isRecord(policyCompilerArtifact) &&
    policyCompilerArtifact.status === "PASS" &&
    policyCompilerArtifact.commit === report.commit &&
    policyCompilerArtifact.cleanCheckout === true;
  ...
  return {
    ...
    policyCompiler: policyCompilerArtifact,
    owner: acceptance.owner ?? null,
    unclosedRisks: acceptance.residualRisks ?? [],
  };
}

export async function writeG20305EvidenceManifest(
  outputDirectory: string,
  report: Readonly<Record<string, unknown>>,
): Promise<void> {
  const acceptance = parseRecord(
    await readFile(resolve(outputDirectory, "g2-03-05-acceptance.json"), "utf8"),
    "G2-03-05 acceptance",
  );
  const policyCompilerArtifact = await readOptionalJson(
    resolve(outputDirectory, "g2-03-05-policy-compiler.json"),
  );
  const manifest = g20305EvidenceManifest(report, acceptance, policyCompilerArtifact);
  await writeFile(
    resolve(outputDirectory, "g2-03-05-evidence-manifest.json"),
    `${JSON.stringify(manifest, null, 2)}\n`,
  );
  if (manifest.status !== "PASS") throw new Error("G2-03-05 evidence manifest is incomplete.");
}

async function checkG20305Evidence(repositoryRoot: string): Promise<void> {
  const policy = await loadPolicy(repositoryRoot);
  const snapshot = await loadSnapshot(repositoryRoot, policy);
  const violations = evaluateG20305EvidenceSnapshot(snapshot, policy);
  ...
  await writeFile(
    resolve(outputDirectory, "g2-03-05-acceptance.json"),
    `${JSON.stringify(artifact, null, 2)}\n`,
  );
  if (violations.length > 0) throw new Error(violations.join(" "));
  process.stdout.write(
    `g2-03-05 acceptance: PASS (${String(policy.requiredRecords.length)} records, ${String(policy.requiredGates.length)} gates)\n`,
  );
}
...
async function loadSnapshot(
  ...
  policy: G20305EvidencePolicy,
): Promise<G20305EvidenceSnapshot> {
  ...
  const priorPolicies = await Promise.all(
    ["g2-02", "g2-03-01", "g2-03-02", "g2-03-03", "g2-03-04"].map(
  ...
  return {
    ...
    policyCompilerArtifact: await readOptionalJson(
      resolve(repositoryRoot, "generated/ci-report/g2-03-05-policy-compiler.json"),
    ),
  };
}

function validateForwardPolicies(snapshot: G20305EvidenceSnapshot, violations: string[]): void {
  ...
  if (!workspacePackages.every((path) => packages.includes(path))) {
    violations.push("Foundation scope does not admit every Policy Compiler package.");
  }
  if (!migrations.includes(migration)) {
    violations.push("Foundation scope does not admit the G2-03-05 migration.");
  }
  ...
  for (const [gate, candidate] of Object.entries(snapshot.priorPolicies)) {
    ...
    if (
      ...
      violations.push(`${gate} forward scope does not admit G2-03-05 Policy Compiler work.`);
    }
  }
}

function validatePolicyCompilerArtifact(
  candidate: unknown,
  currentCommit: string,
  violations: string[],
): void {
  if (!isRecord(candidate)) {
    violations.push("G2-03-05 Policy Compiler artifact is missing.");
    return;
  }
  ...
  if (
    ...
    candidate.qualification !== "REAL_POSTGRES_16_VERSIONED_S3_RELEASE_GATE" ||
    candidate.commit !== currentCommit ||
    candidate.cleanCheckout !== true ||
    candidate.compilerVersion !== "policy-compiler-g2-03-05-v1" ||
    typeof candidate.testVectorCount !== "number" ||
    candidate.testVectorCount < 4 ||
    migrations.historicalPrefix !== 25 ||
    migrations.current !== 26 ||
    !Array.isArray(migrations.applied) ||
    !migrations.applied.includes(26) ||
    ...
    violations.push("G2-03-05 Policy Compiler artifact is incomplete.");
  }
}

function assertPolicy(value: unknown): asserts value is G20305EvidencePolicy {
  if (
    !isRecord(value) ||
    ...
    throw new Error("G2-03-05 evidence policy is invalid.");
  }
}
...
async function loadPolicy(repositoryRoot: string): Promise<G20305EvidencePolicy> {
  const value: unknown = JSON.parse(
    await readFile(resolve(repositoryRoot, "security/g2-03-05-evidence-policy.json"), "utf8"),
  );
  assertPolicy(value);
  return value;
}
...
if (
  ...
  try {
    await checkG20305Evidence(repositoryRoot);
  } catch (error) {
    process.stderr.write(`g2-03-05 acceptance: FAIL (${String(error)})\n`);
    process.exitCode = 1;
  }
}
