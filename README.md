...
- 正式产品实现：**Metadata/Package 控制面已具备真实 OIDC、受限 HTTP 入口、最小 RBAC、PostgreSQL Repository、Release 原子发布、Package Install/Upgrade/Rollback 和进程重启恢复；统一 Gate 已扩展到 25 道**；
- G2-02 Materialization：**G2-02-01～07 已 PASS，已有受管 CSV Ingress、确定 Mapping、永久 Object Identity、Attempt-owned Base、不可见 Current、质量报告、Rejected Artifact、最小血缘和 Owner 行数确认；完整 Gate 仍未实现**；
- 下一唯一允许的工作项：**G2-02-08，落地 PostgreSQL Job/Lease 与独立 Worker 的 Kill/Resume；不得跳到 Index、Cutover、Query 或 UI**。
