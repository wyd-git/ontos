...
| `preflight` | 可信 GitHub Draft PR 的任意高风险变更；必须具备有效 Base/Head、非空 Diff 和至少一个高风险路径 | 除 100k/1m Materialization Clean-room 及其两个依赖资格生成器外的 40 道 Gate      | 只能产生 `PREFLIGHT_PASS`；不能满足 Required Check 或关闭 G2 |
| `full`      | Ready PR 或 main 的高风险变更、每日定时、手动运行、本地默认，或任何无法可信分类的上下文       | 完整 Foundation + Metadata + Materialization + G2-03 共 43 道 Gate               | 按原合同重生 G2-00/01/02 与 G2-03-01～06 Manifest            |
