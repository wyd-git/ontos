...
GitHub Actions 不重新拼装业务命令，只在固定 Node/npm 环境执行 `npm run verify`。远端 Required Check 仍只有一个 `Foundation Gate`；编排器先根据 GitHub 提供的 Base/Head Commit 读取完整 `git diff --no-renames`，再选择三个互斥 Profile 之一。Draft PR 使用不同的 `Foundation Preflight` Check 名称，不能满足分支保护；转为 Ready 会在同一 Head 上触发真正的 `Foundation Gate`。
...
| `preflight` | 可信 GitHub Draft PR 的任意高风险变更；必须具备有效 Base/Head、非空 Diff 和至少一个高风险路径 | 除 100k/1m Materialization Clean-room 及其四个依赖资格生成器外的 41 道 Gate      | 只能产生 `PREFLIGHT_PASS`；不能满足 Required Check 或关闭 G2 |
| `full`      | Ready PR 或 main 的高风险变更、每日定时、手动运行、本地默认，或任何无法可信分类的上下文       | 完整 Foundation + Metadata + Materialization + G2-03 共 46 道 Gate               | 按原合同重生 G2-00/01/02 与 G2-03-01～08 Manifest            |
...
本地直接运行 `npm run verify` 不提供受信 Base/Head，因此始终选择 `full`。`npm run verify:preflight` 仅提供显式的非资格本地预检，建议只在干净 Commit/Worktree 上使用；它在 GitHub Actions 中被代码拒绝，不能通过修改命令行把 Ready PR 降级。每日定时和 GitHub 手动执行同样始终跑完整 Gate，用于发现外部镜像、Advisory、时序和长时运行回归。
...
同时生成简短的 `summary.md`，供 GitHub Job Summary 和人工审查使用。`fast-docs` 另生成 `fast-docs-evidence.json`；`preflight` 另生成 `preflight-evidence.json`，固定 `closesG2Gate=false` 并明文声明不能合并或替代 Clean-room；只有 `full` 才生成 G2-00、G2-01、G2-02 与 G2-03-01～08 Manifest。原始供应链机器输出保存在同一目录。

G2-03-03 把 `g2-03-03-persistence-evidence` 放在 `postgres-integration` 之后：前者必须读取同一运行刚生成的 Persistence 与 Query Lease Artifact，再检查三个 Migration、历史 Scope 前向接纳、Required Record 和 Source Marker。G2-03-04 追加 `runtime-identity-postgres` 与 `g2-03-04-identity-evidence`，用真 OIDC/DPoP/PostgreSQL 和两个独立 API 进程验证 Runtime Identity 与跨进程 Replay。G2-03-05 追加 `policy-compiler-postgres` 与 `g2-03-05-policy-evidence`，用真 PostgreSQL 16/版本化 S3 验证 Compiler、Artifact、最小写入身份和 Release Gate。G2-03-06 复用同一 `policy-compiler-postgres` 环境启动两个 Gateway/Listener，验证 Human/Service/Delegated、精确 Artifact、NOTIFY 与 5 秒撤权，再由 `g2-03-06-policy-evidence` 绑定当次 Artifact。G2-03-07 在同一 100k Object/1m Link Materialization clean-room 中生成 Get/List/Policy/Count/Link 的真实 PostgreSQL 计划，再由后置 `g2-03-07-query-evidence` 检查参数化、Current 绑定、Published Index、零顺序扫描和 Timeout 回收；Preflight 同时跳过这两个强依赖步骤。G2-03-08 追加短时 `runtime-query-postgres`，从空库执行 `0001`～`0028` 并真实 Kill Lease Owner、验证 GC/权限/Retire；后置 `g2-03-08-runtime-evidence` 只在 Full Gate 生成完成资格。full profile 现有 46 道 Gate；最终 Manifest 要求对应 Artifact 与 Report 同 Commit、`cleanCheckout=true` 且每个 Required Gate 恰好 PASS 一次。
...
- 事件：`pull_request` 的 `opened/synchronize/reopened/ready_for_review/converted_to_draft`、`push` 到 `main`、每日 18:00 UTC（上海时间次日 02:00）和 `workflow_dispatch`；不使用 `pull_request_target`；
- Draft PR Job 精确命名为 `Foundation Preflight`；Ready PR、main、定时与手动 Job 精确命名为 `Foundation Gate`，分支保护只接受后者；
