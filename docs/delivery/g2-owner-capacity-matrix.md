...
| Web、SDK、Portability                 | `wyd-git`         | G2-03 只读消费者；G2-05 完整化 | 两 Package 真实集成与独立验收                     |
...
|    3 | G2-03 Query + Policy  |     11–18 工程周 | Identity/Delegation、Policy SQL、Cursor/GC、HTTP 与只读 Web |
|    4 | G2-04 Action          |       3–4 工程周 | Preflight、锁、原子事务、Outbox 故障注入                    |
|    5 | G2-05 Portability     |       2–4 工程周 | 两 Package、完整最小 UI、Function、可发布 SDK、回归         |
|      | **合计**              | **27–44 工程周** | 不含等待外部审查、需求变更或基础设施采购                    |
...
- G2-03 任务包、可行性复审与红队已于 2026-08-17 冻结；下一步收窄为 G2-03-01。

### G2-03 入口重估（2026-08-17）

- [G2-03 任务包](g2-03-query-policy-task-pack.md)拆为 **15 个顺序工作项**；[可行性复审](../reviews/g2-03-task-pack-feasibility.md)与[红队](../reviews/g2-03-task-pack-red-team.md)将 Runtime Identity/Delegation、Policy IR/SQL、Execution Context/Query Lease、OpenAPI Generated Client 和只读 Web 消费者列为承重风险。
...
### G2-03-02 后检查点（2026-08-18）

- G2-03-02 已 PASS，当前进度为 **2/15**；12 类 Query/Policy/Identity/Cursor/Runtime Read 合同、22 个核心错误、两领域/5 Actor Golden、5 路径 OpenAPI Candidate 与私有 Generated Read Client 已冻结。
- Schema、Runtime Parser、OpenAPI 和 Client 由同一字段源驱动；Required/Enum/Limit/Nullability、删除/改名/类型/必填与 Path Mutation 会在 SQL、HTTP、UI 开始前失败。
