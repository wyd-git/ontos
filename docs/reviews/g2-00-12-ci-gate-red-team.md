...
结论：**Conditional Go**。本地唯一入口已真实执行全部 15 个 Gate，Contract、越层依赖、角色越权和 Secret 四类故意失败输入均能被检测；在 GitHub Required Check、严格分支保护和远端 Artifact 通过前，不升级为最终 Go。
...
- **处理：** Workflow Check 固定命名；Actions 最小只读权限并固定完整 SHA。最终关闭条件是 PR 远端 PASS 后写入并复查 Branch Protection。**OPEN — Owner: Repository Owner / Platform**。
...
| Required Check、紧急绕过可审计                                                     | 设计与 Workflow 已实现；远端 Protection 尚待本 PR Check 出现后配置                             | CONDITIONAL |
...
- GitHub 托管 Runner 的首次镜像拉取时间可能波动；30 分钟 Timeout 由首个远端运行验证，不能用删除真实 Smoke 解决。
...
1. PR 上 `Foundation Gate` 远端 PASS，并能下载完整 `generated/ci-report` Artifact；
2. Main Protection 通过 GitHub API 复查为 Strict Required Check、禁止 Force/Delete、Admin Enforced、无常驻 Bypass；
3. Evidence 记录远端 Run、Protection 状态和最后一次提交后的复验结果。
