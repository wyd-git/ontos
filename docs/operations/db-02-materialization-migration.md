...
migrations/db-00/0007 ... 0011  DB-02 当前正式结构
...
2. 导出并保存 `ontos_migration.schema_migrations` 的 `version/name/sha256`；所有已应用版本（最高可到 0011）必须与发布制品逐字一致。
...
- Ledger 恰好连续到 0011，重复运行返回 no-op；
...
- 0007～0011 已支持 Snapshot/Ingress/Job 事实、确定 Identity 和 Attempt-owned Object/Link Base；仍不得在 G2-02-07～14 通过前开启 Current Serving、Cutover 或 GC 流量。
