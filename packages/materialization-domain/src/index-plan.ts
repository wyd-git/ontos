...
export type IndexPlanDigester = (canonicalText: string) => ArtifactDigest;
...
export interface CompiledIndexDefinition {
  name: string;
  physicalSignature: ArtifactDigest;
  ...
  unitCost: number;
  evidenceRefs: string[];
  keys: CompiledIndexKey[];
}

export interface CompiledIndexKey extends BtreeIndexKey {
  direction: "ASC" | "DESC";
...
export const INDEX_PLAN_COMPILER_VERSION = "g2-02-09-v1";
...
export function compileReleaseIndexPlan(
  ...
  for (const objectType of input.objectTypes) {
    ...
    const compiled = compileObjectType(objectType, evidenceCatalog, digestCanonicalText, policy);
  ...
  const planDigest = digest(
    ...
    JSON.stringify({
      ...
      objectTypePlans: compiledObjectTypes
        .map((item) => ({
        ...
        .sort((left, right) =>
          `${left.resourceId}:${left.revisionId}`.localeCompare(
            `${right.resourceId}:${right.revisionId}`,
          ),
        ),
    }),
  );
...
function compileObjectType(
  ...
  const planDigest = calculateObjectTypeIndexPlanDigest(
    input.resourceId,
    input.revisionId,
    compiled,
    digestCanonicalText,
...
export function calculateObjectTypeIndexPlanDigest(
  resourceId: string,
  revisionId: string,
  ...
  return digest(
    ...
    JSON.stringify({
      ...
      entries: indexes
        .map((index) => ({
          name: index.name,
          physicalSignature: index.physicalSignature,
          recipe: index.recipe,
          unitCost: index.unitCost,
          evidenceRefs: [...index.evidenceRefs].sort(),
...
export function validateCompiledIndexDefinition(
  ...
  if (
    ...
    definition.predicate.objectTypeResourceId !== definition.resourceId ||
    definition.predicate.objectTypeRevisionId !== definition.revisionId ||
    definition.predicate.lifecycleState !== "active" ||
    definition.evidenceRefs.length < 1 ||
    new Set(definition.evidenceRefs).size !== definition.evidenceRefs.length ||
    definition.keys.length < 1 ||
    definition.keys.some(
      (key) =>
        !/^[A-Za-z][A-Za-z0-9_]{0,62}$/u.test(key.propertyId) ||
        !["ASC", "DESC"].includes(key.direction) ||
        (key.jsonPath !== undefined && !/^\$\.[A-Za-z][A-Za-z0-9_]*$/u.test(key.jsonPath)),
    )
  ) {
    fail("INDEX_DECLARATION_INVALID", "Persisted Index definition is outside the P0 contract.");
  }
  const declaration: IndexDeclaration =
    definition.kind === "btree"
      ? {
          kind: "btree",
          keys: definition.keys.map(({ propertyId, jsonPath, direction }) => ({
            propertyId,
      ...
      : definition.kind === "gin_trigram"
        ? {
            kind: "gin_trigram",
            propertyId: required(definition.keys[0], "Index key").propertyId,
            evidenceRefs: definition.evidenceRefs,
          }
        : {
            kind: "gin_array",
  ...
  if (
    (definition.kind === "btree" &&
      (definition.keys.length > 3 ||
        definition.unique !== ("unique" in declaration && declaration.unique === true))) ||
    (definition.kind !== "btree" && (definition.keys.length !== 1 || definition.unique))
  ) {
    fail("INDEX_DECLARATION_INVALID", "Persisted Index key shape is invalid.");
  }
  const expectedUnitCost =
    definition.kind === "gin_trigram"
      ? 4
      : definition.kind === "gin_array"
        ? 3
        : definition.unique
          ? 2
          : definition.keys.length;
  const expectedRecipe = recipeFor(declaration, definition.keys);
  const canonical = canonicalIndexSignature(
    {
  ...
  const expectedSignature = digest(digestCanonicalText, canonical);
  const expectedName = physicalIndexName(
  ...
  if (
    definition.unitCost !== expectedUnitCost ||
    definition.recipe !== expectedRecipe ||
    definition.physicalSignature !== expectedSignature ||
    definition.name !== expectedName
  ) {
    fail("INDEX_DECLARATION_INVALID", "Persisted Index definition failed recompilation.");
  }
}
...
function compileDeclaration(
  ...
  let keys: CompiledIndexKey[];
  let unitCost: number;
  if (declaration.kind === "btree") {
    keys = declaration.keys.map((key) => ({
      ...key,
      direction: key.direction ?? "ASC",
      valueType: indexKeyValueType(properties, key),
    }));
  ...
  const physicalSignature = digest(digestCanonicalText, canonical);
  const name = physicalIndexName(
    objectType.resourceId,
    objectType.revisionId,
    declaration,
    physicalSignature,
    digestCanonicalText,
  );
  ...
  return {
    ...
    recipe: recipeFor(declaration, keys),
  ...
  const kindCode =
    declaration.kind === "btree"
      ...
      : declaration.kind === "gin_trigram"
        ? "trgm"
        : "arr";
  const resourceHash = digest(digestCanonicalText, resourceId).slice("sha256:".length);
  const revisionHash = digest(digestCanonicalText, revisionId).slice("sha256:".length);
  const signatureHash = physicalSignature.slice("sha256:".length);
  return `ok_oc_${kindCode}_${resourceHash.slice(0, 10)}_${revisionHash.slice(0, 8)}_${signatureHash.slice(0, 12)}`;
}
...
function canonicalIndexSignature(
  objectType: ObjectTypeIndexPlanInput,
  declaration: IndexDeclaration,
  keys: readonly CompiledIndexKey[],
): string {
  return JSON.stringify({
    ...
    unique: declaration.kind === "btree" && declaration.unique === true,
    scopeKeys:
      declaration.kind === "btree" && declaration.unique === true
    ...
    tieBreaker:
      declaration.kind === "btree" && declaration.unique !== true ? "canonical_primary_key" : null,
  });
}

function indexKeyValueType(
  properties: ReadonlyMap<string, PropertyIndexMetadata>,
  key: BtreeIndexKey,
): PropertyType {
  const property = requireProperty(properties, key.propertyId);
  if (key.jsonPath === undefined) return property.type;
  const path = property.registeredJsonPaths?.find((item) => item.path === key.jsonPath);
  if (path === undefined) {
    fail("INDEX_DECLARATION_INVALID", `JSON path ${key.jsonPath} is not registered.`);
  }
  return path.valueType;
}

function recipeFor(declaration: IndexDeclaration, keys: readonly CompiledIndexKey[]): IndexRecipe {
  if (declaration.kind === "gin_trigram") return "TRIGRAM_GIN";
  if (declaration.kind === "gin_array") return "ARRAY_GIN";
  if (declaration.unique === true) return "UNIQUE_BTREE";
  const type = required(keys[0], "Index key").valueType;
  switch (type) {
    case "string":
      return "BTREE_TEXT";
    case "enum":
      return "BTREE_ENUM";
    case "integer":
      return "BTREE_INTEGER";
    case "decimal":
      return "BTREE_DECIMAL";
    case "date":
      return "BTREE_DATE";
    case "timestamp":
      return "BTREE_TIMESTAMP";
    case "boolean":
      return "BTREE_BOOLEAN";
    case "string[]":
    case "json":
      fail("INDEX_DECLARATION_INVALID", `Unsupported btree value type ${type}.`);
  }
}
...
function assertDigester(digestCanonicalText: IndexPlanDigester): void {
  if (typeof digestCanonicalText !== "function") {
    fail("INDEX_DECLARATION_INVALID", "Index Plan compiler needs a trusted SHA-256 digester.");
  }
}

function digest(digestCanonicalText: IndexPlanDigester, value: string): ArtifactDigest {
  try {
    return parseArtifactDigest(digestCanonicalText(value), "$indexPlanDigest");
  } catch {
    fail("INDEX_DECLARATION_INVALID", "Index Plan digester returned an invalid SHA-256 digest.");
  }
}
